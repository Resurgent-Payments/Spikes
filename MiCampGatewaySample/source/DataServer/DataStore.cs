...
namespace DataServer.Persistence
{
    ...
    public sealed class DataStore : IStreamStoreConnection
    {
            ...
            params EventData[] events)
        {
            ...
            else
            {
                ...
                if (streamExists && expectedVersion == ExpectedVersion.NoStream)
                    throw new ReactiveDomain.WrongExpectedVersionException($"Stream {stream} exists, expected no stream");
                if (!streamExists && (expectedVersion == ExpectedVersion.StreamExists))
                    throw new ReactiveDomain.WrongExpectedVersionException($"Stream {stream} does not exist, expected stream");
                ...
                if (expectedVersion != ExpectedVersion.NoStream &&
                    ...
                    throw new ReactiveDomain.WrongExpectedVersionException(
        ...
        public StreamEventsSlice ReadStreamForward(
                                    ...
                                    UserCredentials credentials = null)
        {
            if (start < 0) throw new ArgumentOutOfRangeException($"{nameof(start)} must be positive.");
            IStream stream;
            lock (_store)
            {
